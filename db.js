...
function readSeed() {
  ...
  for (const k of ['users','courses','payments','applications','materials','assignments','submissions','liveClasses','announcements','posts','notifications','receipts','certificates','teachers','messageLogs','communityMessages']) if (!Array.isArray(d[k])) d[k] = [];
...
async function initStore() {
  ...
  if (result.rowCount === 0) {
    ...
    for (const k of ['users','courses','payments','applications','materials','assignments','submissions','liveClasses','announcements','posts','notifications','receipts','certificates','teachers','messageLogs','communityMessages']) if (!Array.isArray(state[k])) state[k] = [];
