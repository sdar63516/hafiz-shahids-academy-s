...
async function loadPublic(){const r=await fetch('/api/public-state',{cache:'no-store'});const j=await r.json();DB=j&&typeof j==='object'?j:{};DB.settings=(DB.settings&&typeof DB.settings==='object')?DB.settings:{};DB.courses=Array.isArray(DB.courses)?DB.courses:[];DB.posts=Array.isArray(DB.posts)?DB.posts:[];DB.announcements=Array.isArray(DB.announcements)?DB.announcements:[];}
async function loadStudent(){const j=await api('/api/student-state');DB={...DB,...j};DB.settings=(DB.settings&&typeof DB.settings==='object')?DB.settings:{};for(const k of ['courses','materials','assignments','submissions','liveClasses','announcements','posts','notifications','receipts'])if(!Array.isArray(DB[k]))DB[k]=[];current=j&&j.user?j.user:{};current.enrollments=Array.isArray(current.enrollments)?current.enrollments:[];current.purchased=Array.isArray(current.purchased)?current.purchased:[];current.progress=(current.progress&&typeof current.progress==='object')?current.progress:{};localStorage.setItem('hsa_user',JSON.stringify(current));renderHeader();}
...
function renderHeader(){applyBranding();if(!current)return;$('ownerPanel')?.classList.toggle('hidden',current.role!=='superadmin');const name=current.name||'Student';const userName=$('userName');if(userName)userName.textContent=name;const avatar=$('topAvatar');if(avatar)avatar.textContent=name.split(' ').map(x=>x[0]).slice(0,2).join('').toUpperCase();const dot=$('notifDot');if(dot)dot.style.display=(DB.notifications||[]).some(n=>!n.read)?'block':'none'}
...
function watchLesson(cid,lid){const c=course(cid),l=c?.lessons?.find(x=>x.id===lid);if(!l)return;let media='';if(l.type==='upload')media=`<div class="video-frame"><video controls playsinline preload="metadata" controlsList="nodownload" src="${l.video}"></video></div>`;else{const embed=youtubeEmbed(l.video);media=embed?`<div class="video-frame"><iframe src="${embed}" title="${l.title}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`:`<div class="locked"><b>Video link could not be opened.</b><p>Please ask the academy to check this YouTube link.</p></div>`;}$('content').querySelector('.player-placeholder').outerHTML=`<div class="player">${media}<div class="player-body"><b>${l.title}</b><p>${l.description||''}</p><span class="class-type-pill">${l.type==='upload'?'Recorded video':'YouTube class'}</span></div></div>`;toast('Lesson opened. Mark it complete when you finish.');$('content').querySelector('.course-tabs').insertAdjacentHTML('afterend',`<div class="row" style="margin-top:14px"><button class="primary" onclick="completeLesson('${cid}')">Mark Lesson Complete</button></div>`)}
function youtubeEmbed(url){try{const raw=String(url||'').trim();if(!raw)return '';const u=new URL(raw);let id='';if(u.hostname.includes('youtu.be'))id=u.pathname.split('/').filter(Boolean)[0]||'';else if(u.hostname.includes('youtube.com'))id=u.searchParams.get('v')||u.pathname.match(/\/(?:shorts|embed|live)\/([^/?]+)/)?.[1]||'';id=id.replace(/[^a-zA-Z0-9_-]/g,'');return id?`https://www.youtube.com/embed/${id}?rel=0&modestbranding=1&playsinline=1`:''}catch{return ''}}
