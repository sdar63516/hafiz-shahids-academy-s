...
function applyBranding(){const logo=DB?.settings?.logo||'/logo.svg'; for(const id of ['brandMark','loginBrandMark']){const e=$(id);if(e){e.innerHTML=`<img src="${logo}" alt="Academy logo">`;e.classList.add('logo-mark');}}}
...
$('nameForm').addEventListener('submit',async e=>{
  ...
  try{
    ...
    toast(j.isNew?`Your new Student ID is ${j.studentId}`:`Student ID: ${j.studentId}`);
...
function renderSettings(){$('content').innerHTML=`<div class="page-head"><div><div class="eyebrow">ACCOUNT</div><h1>Settings</h1><p>Simple controls for your demo account.</p></div></div><div class="card"><h3>Security</h3><p class="small">Phone OTP is your login method. If the same verified number is used to login on another device, the previous session is invalidated automatically.</p><button class="outline" onclick="logout()">Sign out this device</button></div>`}
...
async function boot(){await loadPublic();applyBranding();if(studentToken){try{await loadStudent();$('loginGate').classList.add('hidden');$('app').classList.remove('hidden');go('home');return}catch{localStorage.removeItem('hsa_student_token');studentToken=''}}}
boot();
